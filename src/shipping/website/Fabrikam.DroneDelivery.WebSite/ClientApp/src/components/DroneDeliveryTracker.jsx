...
export const DroneDeliveryTracker = () => {
    ...
    const onTrack = () => {
        const locations = droneDeliveryService.fetchCompleteTrackingInfo(trackingId);
        const result = populateLocations(locations);
        setDroneLocations(result);

    }
    const handleInput = (event) => {
        setTrackingId(event.target.value)
    }
    const populateLocations = (location) => {
        let locationPoints = [
            {
                location: [location.pickup.latitude, location.pickup.longitude],
                option: { color: 'red', title: 'pickup' }
            },
            {
                location: [location.pickup.latitude, location.pickup.longitude],
                option: { color: 'green', title: 'Current location' }
            ...
            {
                location: [location.pickup.latitude, location.pickup.longitude],
                option: { color: 'green', title: 'Dropoff' }
            }
        ]
...
return (
    <div>
        ...
        <div style={{ paddingBottom: 10 }}>
            <input type="text" onChange={handleInput} placeholder="Enter tracking id"></input>
            <button type="primary" onClick={onTrack}>Track</button>
        </div>
